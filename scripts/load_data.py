...
import pytz
def getoffset(x):
	datetime.now(pytz.timezone(x)).strftime('%z')
	res = pytz.timezone(x).localize(datetime(2011,1,1)).strftime('%z')
...
def get_row_store_status(reader):
	...
	for row in reader :
		...
		t = datetime.strptime(row[2][:19], '%Y-%m-%d %H:%M:%S') 		
		x = StoreStatus(storeid=row[0], status=row[1] , datetime_utc = t )
		lis.append( x )
	return lis
...
def get_row_store_timezone(reader):
	lis = []
	for row in reader :
		x = StoreTimezone(storeid=row[0],timezone=row[1] , utcoffset = getoffset(row[1]) )   
		lis.append( x )
	return lis
